...
/*Problem - Create a chocolate boiler which will take chocolate and mild bring
them to boil and then pass them on the next phase of making chocolate bars.*/

class ChocolateBoiler {
public:
public
  ChocolateBoiler() : empty(true), boiled(true) {}
...
global point of access to it. To get the instance of Singleton class we don't
instantiate one, we just ask for the instance. (Feels like a global variable
inside a namespace)

Uses :
Thread pools, Caches, Dialog boxes, objects used for logging, object used for
resources
...
/**Pros
        ...
        4) Memory is only allocated if you actually use the Singleton
(implementation based) 5) Subclassing
